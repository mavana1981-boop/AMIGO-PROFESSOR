...
class AtividadeIndividualizada(db.Model):
    """Atividade específica para aluno(s) dentro de um plano de aula."""
    __tablename__ = "atividades_individualizadas"
    id          = db.Column(db.Integer, primary_key=True)
    plano_id    = db.Column(db.Integer, db.ForeignKey("planos_aula.id"), nullable=False)
    descricao   = db.Column(db.Text, nullable=False)
    alunos_ids  = db.Column(db.Text)   # JSON list de IDs dos alunos
    criado_em   = db.Column(db.DateTime, default=datetime.utcnow)

    plano = db.relationship("PlanoAula", backref=db.backref("atividades_individualizadas", lazy=True, cascade="all, delete-orphan"))
